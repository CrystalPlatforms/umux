...
The formal spec is the master PRD at `plans/umux-prd.md` (user stories, constraints, Roadmap); `README.md` is the user-facing guide (install, build). Keep both in sync when scope changes. Each roadmap version also has a per-version PRD extract at `plans/umux-vX.Y.Z-prd.md` (v1.5.0, v1.6.0, v1.7.0, v1.8.0, v1.9.0, v2.0.0). **Implementation status: the import-&-CLI scope is fully shipped** (latest release v1.5.4, 2026-09-07 — Linux, Windows, macOS; unlimited panels, agent status, Settings, session restore, cmux import, offline `umux` CLI, in-app updates; it was planned as roadmap "v1.2.0" — **versions renumbered 2026-09-02** so roadmap numbers continue the tag sequence: v1.5.0 → v1.6.0 → v1.7.0 → v1.8.0 → v1.9.0). **Next build targets: v1.6.1 and v1.7.0** (v1.5.0–v1.5.4 shipped 2026-09-02 → 2026-09-07 — eight-color palette for workspaces/tabs/groups, port-click open + copy, inline rename pencil removed from workspace rows, Windows smoothness + macOS repair fixes, the xterm DECRQM black-screen fix; **v1.6.0, released 2026-09-10: cross-platform shell picker** — default-shell choice in Settings from auto-detected installed shells + custom entry on Windows/Linux/macOS, and an arrow beside "+ New tab" for a per-tab shell, **hidden when only one shell is detected**; local tabs only, SSH unchanged; plus two default-off Settings switches — hide git branch on tab rows, per-tab working directories on workspace rows; the package was renumbered v1.9.0 → v1.6.0 and extended beyond Windows on **2026-09-07**, swapping with the polish package; the sidebar drag-resize fix with persisted width **moved to v1.6.1 on 2026-09-10**, issue #79; see `plans/umux-v1.6.0-prd.md`), then **v1.7.0** (umux Terminal TUI + live CLI/socket at full parity + herdr importer — the release is gated on the TUI working on all three platforms), followed by v1.8.0 (agent UX + native menus) and **v1.9.0** (lucide-react icons 1:1, spring press feedback on every interactive element — standing rule, story #101 — and the pinned-tab rebuild: pin replaces the close X, pinned tabs unclosable until unpinned and always first in the tab bar; moved from v1.6.0 in the 2026-09-07 swap) per the PRD Roadmap.
