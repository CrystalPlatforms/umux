...
- Out of scope on `main`: browser integration, git integration, custom theming, cross-machine sync. Plugins are Beyond the Ecosystem; the background session server is **v1.7.0 (umux Storestation)**; mobile (NativeApps) and remote access live in the Ecosystem on the `development` branch (see PRD Roadmap).
