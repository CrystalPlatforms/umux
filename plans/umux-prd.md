...
**Status:** the import-&-CLI scope fully shipped (tags v1.0.3 + v1.0.4, latest release v1.0.4, 2026-08-31; planned at the time as roadmap "v1.2.0"); roadmap reshuffled 2026-08-28 after a new discovery session (import from cmux/herdr, `umux` CLI, umux Terminal TUI); extended 2026-08-29 with in-app updates (issue #55) and native menus (issue #56); extended 2026-08-31 after the "versions cleanup" discovery — the Terminal release confirmed (Ctrl+B prefix, release gated on the TUI working on all three platforms, full live-CLI parity, herdr importer stays) and a shell-picker package created (issue #67 + sidebar/metadata polish); extended 2026-09-01 after the "UI patch releases" discovery — a colors release (workspace/tab/group colors, port-click open, rename cleanup) and a polish release (lucide-react icons, spring press feedback everywhere, pinned-tab rebuild); **renumbered 2026-09-02** so roadmap versions continue the tag sequence — **v1.5.0** (colors), **v1.6.0** (polish), **v1.7.0** (umux Terminal + live control), **v1.8.0** (agent UX + native menus), **v1.9.0** (Windows shell picker + sidebar polish) — see Roadmap; **redefined 2026-09-04** after the "umux Ecosystem" discovery (the ecosystem scope now lives on the `development` branch — see the note in the umux Storestation section below); **swapped 2026-09-07** — the shell-picker package (issue #67) renumbered v1.9.0 → **v1.6.0** and extended from Windows-only to local tabs on all platforms (dropdown arrow hidden when only one shell is detected), while the polish package (lucide icons, press feedback, pinned tabs) moved to **v1.9.0**; build order now v1.6.0 → v1.7.0 → v1.8.0 → v1.9.0; **renumbered again 2026-09-12** (PO decision): **v1.7.0 is now umux Storestation** (moved out of the ecosystem; builds on branch `core`, plus a landing-page revamp with docs) and the former v1.7.0/v1.8.0/v1.9.0/v2.0 packages were **split into smaller releases** — v1.8.0 live CLI/socket, v1.9.0 umux Terminal (TUI), v2.0.0 Agents View (+ unread markers #56/#57), v2.1.0 herdr importer + lucide icons + press feedback, v2.2.0 command palette + shortcut rebinding, v2.3.0 native menus + pinned tabs, v2.4.0 teammates-as-panes, v2.5.0 **SSH View** (new discovery 2026-09-12: sidebar view switcher [Local][Agents][SSH], remote umux control over SSH, remote install/update), v2.6.0 multi-window (stories #137–#138), v2.7.0–v2.9.0 reserved ("wyjdzie w praniu"); **extended 2026-09-12 (afternoon):** accent color + custom colors added to v2.1.0 (stories #139–#140) and multi-window made the concrete v2.6.0. **Everything umux Ecosystem lives on the `development` branch** — this file (main) keeps only umux Storestation, which ships as v1.7.0. **Rebranded 2026-09-20 (PO decision):** the v1.7.0 daemon is named **umux Storestation**; the name **umux Core** now belongs to the new **v1.7.5** Always-On feature (stories #141–#143) — older mentions of "umux Core" in this file's history meant today's Storestation.
...
44. As a developer, I want umux to restore my workspaces, panels, layout, working directories, and shells when I reopen the app, so that a restart costs me seconds, not minutes. (Running processes are not resumed — see Roadmap v1.7.0, umux Storestation, for a background daemon.)
...
*(Note: the offline subset — notify plus the definition CRUD below — shipped as tags v1.0.3–v1.0.4; v1.8.0 adds the live surface through the running app's local socket at full parity with the app (2026-08-31). Headless/daemon operation is **v1.7.0, umux Storestation** — it now precedes the live CLI (renumbered 2026-09-12).)*
...
136. As a visitor, I want an expanded umux landing page — a documentation section and more content beyond the current one-pager — so that I can learn and adopt umux without digging through the repo. *(v1.7.0 alongside umux Storestation; exact scope decided at v1.7.0 planning, 2026-09-12.)*
...
### umux Storestation — v1.7.0 (branch `core`) — added 2026-09-04 as part of the ecosystem scope; **v1.7.0 since 2026-09-12**

> Ecosystem on dev. Shipping v3.0.0. This file has umux Storestation only.

105. As a developer, I want Storestation to keep my terminal sessions alive after I close umux Desktop or umux Terminal, so that a closed window never kills a running agent or long job.
106. As a developer, I want `umux attach` to reattach Desktop, Terminal, or the CLI to Storestation's living sessions, so that coming back costs seconds and loses nothing.
107. As a user, I want Storestation OFF to leave the desktop app behaving exactly as today, so that the daemon is pure opt-in.
108. As a user, I want a dedicated Storestation section in Settings (daemon on/off, autostart on/off, running status) styled like the import wizard, so that all ecosystem controls live in one obvious place.
109. As a user, I want Storestation to start automatically at login when autostart is enabled, so that remote access reaches my machine even after a reboot.
110. As a developer, I want stopping Storestation to terminate its shells cleanly and a crashed Storestation's leftovers to be cleaned on the next start, so that the daemon never litters my system.
111. As a developer, I want the CLI's live commands to work against Storestation while no window is open, so that scripts and agents can drive umux headlessly.
112. As a developer, I want umux Terminal (TUI) to be able to attach to the same Storestation sessions as the desktop app, so that both are interchangeable views on the same work. *(Store separation (now v1.9.0) untouched; the attach is v1.7.0 scope and completes when the TUI ships.)*
...
### umux Core (Always-On) — v1.7.5 — added 2026-09-20 (discovery with the PO; the v1.7.0 daemon was rebranded umux Storestation the same day and the name "umux Core" was assigned to this feature)
...
**Planned v1.7.0 — umux Storestation (re-scheduled out of the ecosystem on 2026-09-12; full detail in [`umux-v1.7.0-prd.md`](./umux-v1.7.0-prd.md)):**

17. **SessionCore** *(deep module)* — One session-management interface with two interchangeable drivers: *in-process* (today's behavior, Storestation OFF) and *daemon-client* (all operations proxied to umux Storestation over the local socket, Storestation ON). Desktop/TUI/CLI become views; the UI never knows which driver is active. The local-socket protocol is **defined by umux Storestation itself (v1.7.0)** and is the declared foundation for the desktop CliGateway (v1.8.0).
18. **umux Storestation** *(deep module)* — Headless Rust binary owning PTYs and session state: serves the local socket API, survives app close, optional per-OS autostart (mechanism per platform at /carve), clean shutdown with no orphan shells, crash-leftover cleanup on next start. Shipped inside the existing installers — no separate install. Built on branch `core` (PO decision 2026-09-12).
19. **Settings → Storestation section** — Import-wizard-style section: daemon on/off (default off), autostart on/off, live status; the paired-devices list with revoke fills in with the Bridge work on the `development` branch.

**Planned v1.7.5 — umux Core (Always-On device; discovery 2026-09-20 with the PO, decision on the version number is the PO's):**

- **umux Core (Always-On device)** — an optional Settings switch (off by default): when on, the daemon holds an OS power assertion so the **machine — not the screen** — stays awake even with every umux window closed and zero sessions live; the block releases on daemon stop / toggle-off. Best-effort per platform: reliable on macOS with AC power; where the OS refuses (Windows/Linux lid-close action, macOS on battery) umux shows **instructions** instead of failing silently — no admin/root paths. Limits visible in `umux status --json` (`sleepPrevented`). The daemon (umux Storestation) is the single holder; toggling Core on spawns it if absent. Out of scope: wake-on-LAN, schedules, auto-resume after a forced sleep (wake-time rebind is v1.7.0 scope); remote shutdown arrives with the Ecosystem. Full detail in [`umux-v1.7.5-prd.md`](./umux-v1.7.5-prd.md).

**Planned v1.8.0 — live CLI + socket (was v1.7.0 scope; renumbered 2026-09-12):**

20. **CliGateway** *(was v1.1 scope, then v1.7.0)* — Local socket server inside the running app exposing the live surface at **full parity with the app** (2026-08-31: every action the UI supports, not a curated subset) to the CLI and directly to agents; the CLI and the socket expose the same surface, speaking the Storestation socket protocol (v1.7.0). Detailed design at /carve time.

**Planned v1.9.0 — umux Terminal, TUI (was v1.7.0 scope; renumbered 2026-09-12):**

21. **umux Terminal (TUI)** — A terminal-native frontend (Rust, no webview): sidebar with workspaces, tabs, unlimited panels, tmux-style prefix shortcuts (prefix **Ctrl+B**, confirmed 2026-08-31) plus mouse support. Reuses PtyService and OscParser; shows agent status in the sidebar and panel titles. Keeps its **own** store (separate state from the desktop, decided 2026-08-28). Closing the terminal ends its sessions **unless Storestation is on** (the v1.7.0 daemon; TUI-attach story #112 completes here). Ships on all three platforms; the v1.9.0 release waits until the Windows (ConPTY) TUI works too (2026-08-31 — replaces the old "Linux + macOS first").
...
- **Attach (v1.7.0, umux Storestation):** Desktop/TUI/CLI SessionCore daemon-client → Storestation local socket.
...
- Development model (PO decisions 2026-09-04 + 2026-09-12): **umux Storestation** is built on a dedicated long-lived **`core` branch** and released from `main` as v1.7.0 like any other version; the rest of the ecosystem (Bridge, PWA, NativeApps, the v3.0.0 finale) lives on the **`development` branch** with its own PRD there. Test builds run against an **isolated data directory** (separate store, socket, sessions — e.g. `~/.umux-test`) so a test ecosystem never touches a daily-use umux. Every ecosystem feature is additive: with Storestation never enabled, the app behaves exactly like the v1.9.x app. Plugins, marketplace, and the browser pane stay **Beyond the Ecosystem**; cross-machine workspace **synchronization** stays out of scope (remote access is control, not sync).
...
- The local-socket protocol **defined by umux Storestation (v1.7.0)** proves extensible into the desktop CliGateway (v1.8.0) without a rewrite (it is the declared foundation — since 2026-09-12 Storestation precedes the live CLI, so Storestation defines the protocol).

## Tradeoffs Considered

- **Background daemon in v1.7.0** — rejected: the single most complex component (autostart, crash recovery, secure socket); deferred to v2.0 after being explained to the PO (2026-08-28). **Superseded 2026-09-12:** the PO pulled the daemon forward — it ships as v1.7.0 umux Storestation, now the first ecosystem piece to land.
...
- **v1.7.0 (umux Storestation):** with Storestation ON, Adam closes the desktop mid-agent-run — the agent keeps running (`umux status` answers against Storestation, output still growing); `umux attach` brings the view back; with Storestation OFF everything matches the v1.9.x behavior; stopping Storestation leaves zero orphan shells (process audit); a hard-killed Storestation leaves no garbage after the next start; autostart launches Storestation after a reboot; the Storestation Settings section's toggles persist. HITL order (PO decision 2026-09-12): **macOS and Windows first, Ubuntu after**. The landing-page revamp is verified on the live site when its scope lands.
...
- **v1.9.0 (umux Terminal):** Adam drives the TUI over SSH on Ubuntu (sidebar, tabs, panes, Ctrl+B-prefixed shortcuts, mouse); agent statuses in the TUI match the desktop app; export/import moves setups between Desktop and Terminal in both directions; with Storestation on, the TUI attaches to the same live sessions (#112). *(Release gate, 2026-08-31: all three platforms before shipping.)*
...
- **v1.7.0 — umux Storestation + landing-page revamp (rescheduled out of the ecosystem 2026-09-12; branch `core`):** the optional umux Storestation daemon (off by default) keeping sessions alive past window close — SessionStorestation's in-process/daemon-client drivers, `umux attach`, a dedicated Storestation section in Settings (daemon/autostart switches, live status), autostart at login, clean shutdown + crash-leftover cleanup, the CLI driving Storestation headlessly, Storestation shipping inside the normal installers — plus the expanded landing page (a docs section and more; exact scope set at v1.7.0 planning, story #136). HITL order: macOS + Windows first, Ubuntu after. Extract: [`umux-v1.7.0-prd.md`](./umux-v1.7.0-prd.md).
- **v1.8.0 — live CLI + local socket (was v1.7.0 scope):** `umux list`/`status` and the control commands against the running app, plus the local socket API for agents — at **full parity with the app** (stories #53–#55, #87); the socket speaks the protocol umux Storestation (v1.7.0) defined.
- **v1.9.0 — umux Terminal, TUI (was v1.7.0 scope):** the full TUI (sidebar/tabs/panes, tmux-style prefix — Ctrl+B — + mouse, OSC agent statuses in sidebar and panel titles), separate Desktop/Terminal states with export/import in both directions, and TUI attach to Storestation sessions (#112). **Release gate (2026-08-31): ships only once the TUI works on all three platforms — Linux, macOS, Windows (ConPTY).**
...
- **umux Ecosystem — `development` branch, final release v3.0.0 (no deadline):** umux Bridge, umux Application (PWA), umux NativeApp for Android & iOS and the finale — fully documented in the Ecosystem PRD on the `development` branch, not here. *(The Ecosystem's session-keeper, umux Storestation, ships earlier as v1.7.0 above.)*
...
- Plugin system and marketplace — **Beyond 2.0** (moved out of v2.0 by the PO, 2026-09-04). A background session server (umux Storestation) is v1.7.0 scope (see Roadmap).
...
- A background daemon / headless umux **before v1.7.0** — from v1.7.0, umux Storestation is the optional way to outlive windows; the v1.9.0 umux Terminal with Storestation off lives only as long as its terminal window is open.
