# umux v1.3.0 — PRD (umux Terminal + live control)

**Status:** planned — **the next release to build**
...
umux is GUI-only, which excludes terminal-native users: people who live over SSH, work on headless machines, or simply prefer the keyboard-first multiplexer workflow. Meanwhile the v1.2.0 CLI is offline-only — scripts and AI agents can inspect and edit *saved definitions*, but cannot see or steer a *running* app. And while cmux users have an import path, herdr users — the closest audience umux has — are left out.

## Solution

v1.3.0 ships three things that share one engine:
...
- **Background daemon in v1.3.0** — rejected: the single most complex component (autostart, crash recovery, secure socket); deferred to v2.0 (explained to the PO, 2026-08-28).
...
- **herdr importer earlier than v1.3.0** — rejected: unofficial format; it ships alongside the TUI (re-confirmed 2026-08-31).
