...
describe("umux landing page (Issue #35, Phase 11)", () => {
    ...
    // --- Visual/UX/performance refresh (Codebuff pass, 2026-09-05) --------
    // Dark-only page polish + a faster first paint: the visual language
    // (colors, cards, fonts) is untested, but these are the non-negotiable
    // parts — declared dimensions (no layout shift), WebP-first screenshots
    // with a guaranteed PNG fallback, a dark theme-color for the mobile
    // browser chrome, and platform-aware ordering of the hero downloads.

    it("declares a dark theme color and explicit screenshot dimensions", () => {
        const doc = parse();
        expect(
            doc.querySelector('meta[name="theme-color"]')?.getAttribute("content"),
        ).toBe("#0d1117");
        const media = [
            [".demo-frame img", "1596", "898"],
            [".feature-img", "1588", "890"],
        ] as const;
        for (const [sel, w, h] of media) {
            for (const img of doc.querySelectorAll(sel)) {
                expect(img.getAttribute("width"), `${sel} must declare width`).toBe(w);
                expect(img.getAttribute("height"), `${sel} must declare height`).toBe(h);
            }
        }
    });

    it("serves WebP screenshots with the PNG fallback intact", () => {
        const doc = parse();
        const sources = [...doc.querySelectorAll("picture source[type=\"image/webp\"]")];
        expect(sources, "expected three WebP sources").toHaveLength(3);
        const srcsets = sources.map((s) => s.getAttribute("srcset"));
        for (const name of ["umux-first.webp", "umux-agent.webp", "umux-session.webp"]) {
            expect(
                srcsets.some((v) => v === `assets/${name}`),
                `missing WebP source for ${name}`,
            ).toBe(true);
        }
        // The <img> must still resolve to the real PNG (and its inline
        // placeholder fallback) so nothing regresses when WebP is absent.
        for (const img of doc.querySelectorAll("picture img")) {
            expect(img.getAttribute("src")).toMatch(/\.png$/);
            expect(img.getAttribute("onerror")).toContain("data:image/svg+xml");
        }
    });

    it("orders the hero downloads by the visitor's platform", () => {
        // Conversion touch: macOS visitors should see the macOS button first,
        // Windows visitors the Windows one; everyone else keeps the authored
        // macOS-first order (Linux users get the dropdown next to it anyway).
        const firstPlatform = (ua: string): string | null => {
            const win = new JSDOM(html, {
                runScripts: "dangerously",
                url: "https://umux.pages.dev/",
                userAgent: ua,
            }).window.document;
            const row = win.querySelector(".downloads");
            return row?.firstElementChild?.getAttribute("data-platform") ?? null;
        };
        expect(firstPlatform("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)")).toBe(
            "macos",
        );
        expect(
            firstPlatform("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"),
        ).toBe("windows");
        expect(
            firstPlatform("Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36"),
        ).toBe("macos");
    });
});
